...
#Region "    Open/Save Files"
    ...
    Sub SaveF(ByVal caminho As String, ByRef form As FormFlowsheet)
        ...
        'My.Application.ActiveSimulation = 
        My.Application.ActiveSimulation = form
...
#End Region
    ...
    Private Sub GenerateInputFileToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RunToolStripMenuItem.Click
        ...
        Dim format = New CultureInfo("en-US")
        ...
        If SaveFileDialog2.ShowDialog() = DialogResult.OK Then
            ...
            generate.WriteLine("* ==== Unit Selection ===== [INPUT] [OUTPUT] - si/british")
            ...
            If (My.Application.ActiveSimulation.FormPlotReqest.cboRestartPlotSettings.SelectedText.Length > 0) Then
            ...
            generate.WriteLine("105 " & My.Application.ActiveSimulation.FormInitialSettings.txtCPURemaining1.Text & " " & My.Application.ActiveSimulation.FormInitialSettings.txtCPURemaining2.Text & " " & My.Application.ActiveSimulation.FormInitialSettings.txtCPUTimeAllocated.Text)
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.Tank) In ChildParent.Collections.CLCS_TankCollection
                    ...
                    output = ((((((((kvp.Value.UID & "0101 " & kvp.Value.FlowArea.ToString("F", format) & " ") & kvp.Value.LengthofVolume.ToString("F", format) & " ") & kvp.Value.VolumeofVolume.ToString("F", format) & " ") & kvp.Value.Azimuthalangle.ToString("F", format) & " ") & kvp.Value.InclinationAngle.ToString("F", format) & " ") & kvp.Value.ElevationChange.ToString("F", format) & " ") & kvp.Value.WallRoughness.ToString("F", format) & " ") & kvp.Value.HydraulicDiameter.ToString("F", format) & " ") & "0000000"
                    generate.WriteLine(output)
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.SingleJunction) In ChildParent.Collections.CLCS_SingleJunctionCollection
                    ...
                    If kvp.Value.EnterVelocityOrMassFlowRate = False Then
                        output = (((kvp.Value.UID & "0201 " & "0" & " ") & kvp.Value.InitialLiquidVelocity.ToString("F", format) & " ") & kvp.Value.InitialVaporVelocity.ToString("F", format) & " ") & "0.0"
                    ElseIf kvp.Value.EnterVelocityOrMassFlowRate = True Then
                        output = (((kvp.Value.UID & "0201 " & "1" & " ") & kvp.Value.InitialLiquidMassFlowRate.ToString("F", format) & " ") & kvp.Value.InitialVaporMassFlowRate.ToString("F", format) & " ") & "0.0"
                    End If
                    generate.WriteLine(output)
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.TimeDependentJunction) In ChildParent.Collections.CLCS_TimeDependentJunctionCollection
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, JunctionDatavelocity) In kvp.Value.JunctionsData.JunctionDatavelocity
                        output = kvp.Value.UID & "0" & "20" & Counter & " " & kvp2.Value.TimeVelocity.ToString("F", format) & " " & kvp2.Value.LiquidVelocity.ToString("F", format) & " " & kvp2.Value.VaporVelocity.ToString("F", format) & " 0.0"
                        generate.WriteLine(output)
                        Counter = Counter + 1
                    Next kvp2
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.Valve) In ChildParent.Collections.CLCS_ValveCollection
                    ...
                    ElseIf kvp.Value.MomentumFlux.ToString = "Do_not_use_Momentum_Flux" Then
                        output7 = "3"
                    End If
                    output = kvp.Value.FromComponent & CInt(kvp.Value.FromVolume).ToString("D2") & "000" & kvp.Value.FromDirection & " " & kvp.Value.ToComponent & CInt(kvp.Value.ToVolume).ToString("D2") & "000" & kvp.Value.ToDirection & " " & kvp.Value.JunctionArea.ToString("F", format) & " " & kvp.Value.FflowLossCo.ToString("F", format) & " " & kvp.Value.RflowLossCo.ToString("F", format) & " " & output1 & output2 & output3 & output4 & output5 & output6 & output7
                    ...
                    ElseIf kvp.Value.ValveType.ValveTypeName = "chkvlv" Then
                        ...
                        Else
                            output1 = "0"
                        End If
                        output = output1 & " " & kvp.Value.ValveType.checkPosition & " " & CDbl(kvp.Value.ValveType.checkbackpressure).ToString("F", format) & " " & CDbl(kvp.Value.ValveType.checkleakratio).ToString("F", format)
                    ...
                    ElseIf kvp.Value.ValveType.ValveTypeName = "inrvlv" Then
                        output = kvp.Value.ValveType.inertialLatchoption &
                        " " & kvp.Value.ValveType.inertialInitialposition &
                        " " & CDbl(kvp.Value.ValveType.inertialbackpressure).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.inertialLeakratio).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.inertialinitialangle).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.inertialminangle).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.inertialmaxangle).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.inertialmomentinertia).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.inertialangularvelocity).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.inertialmomentlength).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.inertialradius).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.inertialmass).ToString("F", format)
                    ElseIf kvp.Value.ValveType.ValveTypeName = "mtrvlv" Then
                        output = kvp.Value.ValveType.pmotor1 &
                        " " & kvp.Value.ValveType.pmotor2 &
                        " " & CDbl(kvp.Value.ValveType.pmotor3).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.pmotor4).ToString("F", format) &
                        " " & kvp.Value.ValveType.pmotor5
                    ...
                    ElseIf kvp.Value.ValveType.ValveTypeName = "rlfvlv" Then
                        output = kvp.Value.ValveType.prel1 &
                        " " & CDbl(kvp.Value.ValveType.prel2).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel3).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel4).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel5).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel6).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel7).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel8).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel9).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel10).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel11).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel12).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel13).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel14).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel15).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel16).ToString("F", format) &
                        " " & CDbl(kvp.Value.ValveType.prel17).ToString("F", format)
                    End If
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, ValveCSUB) In kvp.Value.ValveType.ProValveCSUB
                        generate.WriteLine(kvp.Value.UID & "04" & counter.ToString("D2") & " " & kvp2.Value.NormalizedFlowArea.ToString("F", format) & " " & kvp2.Value.ForwardCSUBV.ToString("F", format) & " " & kvp2.Value.ReverseCSUBV.ToString("F", format))
                        counter = counter + 1
                    Next kvp2
                    univID = univID + 1
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.Pump) In ChildParent.Collections.CLCS_PumpCollection
                    ...
                    output = ((((((kvp.Value.UID & "0101 " & kvp.Value.FlowArea.ToString("F", format) & " ") & kvp.Value.LengthofVolume.ToString("F", format) & " ") & kvp.Value.VolumeofVolume.ToString("F", format) & " ") & kvp.Value.Azimuthalangle.ToString("F", format) & " ") & kvp.Value.InclinationAngle.ToString("F", format) & " ") & kvp.Value.ElevationChange.ToString("F", format) & " ") & "0"
                    generate.WriteLine(output)
                    ...
                    If kvp.Value.EnterVelocityOrMassFlowRate = False Then
                        output = (((kvp.Value.UID & "0201 " & "0" & " ") & kvp.Value.InitialLiquidVelocity.ToString("F", format) & " ") & kvp.Value.InitialVaporVelocity.ToString("F", format) & " ") & "0"
                        generate.WriteLine(output)
                    ElseIf kvp.Value.EnterVelocityOrMassFlowRate = True Then
                        output = (((kvp.Value.UID & "0201 " & "1" & " ") & kvp.Value.InitialLiquidMassFlowRate.ToString("F", format) & " ") & kvp.Value.InitialVaporMassFlowRate.ToString("F", format) & " ") & "0"
                        generate.WriteLine(output)
                    End If

                    If kvp.Value.OEnterVelocityOrMassFlowRate = False Then
                        output = (((kvp.Value.UID & "0202 " & "0" & " ") & kvp.Value.OInitialLiquidVelocity.ToString("F", format) & " ") & kvp.Value.OInitialVaporVelocity.ToString("F", format) & " ") & "0"
                        generate.WriteLine(output)
                    ElseIf kvp.Value.OEnterVelocityOrMassFlowRate = True Then
                        output = (((kvp.Value.UID & "0202 " & "1" & " ") & kvp.Value.OInitialLiquidMassFlowRate.ToString("F", format) & " ") & kvp.Value.OInitialVaporMassFlowRate.ToString("F", format) & " ") & "0"
                        generate.WriteLine(output)
                    End If
                    'pump index
                    ...
                    generate.WriteLine(kvp.Value.UID & "0302 " & kvp.Value.Ratedpumpvelocity.ToString("F", format) & " " & kvp.Value.RatioRatedVelocity.ToString("F", format) & " " & kvp.Value.RatedFlow.ToString("F", format) & " " & kvp.Value.RatedHead.ToString("F", format) & " " & kvp.Value.RatedTorque.ToString("F", format) & " " & kvp.Value.MomentofInertia.ToString("F", format) & " " & kvp.Value.RatedDensity.ToString("F", format) & " " & kvp.Value.RatedMotorTorque.ToString("F", format) & " " & kvp.Value.TF2.ToString("F", format) & " " & kvp.Value.TF0.ToString("F", format) & " " & kvp.Value.TF1.ToString("F", format) & " " & kvp.Value.TF3.ToString("F", format))
                    ...
                    If output2 = 0 Then
                        ...
                        For Each kvp2 As KeyValuePair(Of Integer, pumpdata201) In kvp.Value.PumpData.Propumpdata201
                            generate.WriteLine(kvp.Value.UID & "30" & counter.ToString("D2") & " " & kvp2.Value.Voidfraction1.ToString("F", format) & " " & kvp2.Value.head.ToString("F", format))
                            counter = counter + 1
                        Next kvp2
                        ...
                        For Each kvp2 As KeyValuePair(Of Integer, pumpdata202) In kvp.Value.PumpData.Propumpdata202
                            generate.WriteLine(kvp.Value.UID & "31" & counter.ToString("D2") & " " & kvp2.Value.Voidfraction2.ToString("F", format) & " " & kvp2.Value.Torque.ToString("F", format))
                            counter = counter + 1
                        Next kvp2
                    End If
                    ...
                    If output5 = 0 Then
                        ...
                        For Each kvp2 As KeyValuePair(Of Integer, pumpdata501) In kvp.Value.PumpData.Propumpdata501
                            generate.WriteLine(kvp.Value.UID & "61" & counter.ToString("D2") & " " & kvp2.Value.SearchVariable.ToString("F", format) & " " & kvp2.Value.PumpVelocity.ToString("F", format))
                            counter = counter + 1
                        Next kvp2
                    End If
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.pipe) In ChildParent.Collections.CLCS_PipeCollection
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, PipeJunctions) In kvp.Value.Profile.Junctions
                        If (counter < 10) Then
                            output = kvp.Value.UID & "090" & counter & " " & kvp2.Value.FflowLossCo.ToString("F", format) & " " & kvp2.Value.RflowLossCo.ToString("F", format) & " " & counter
                        Else
                            output = kvp.Value.UID & "09" & counter & " " & kvp2.Value.FflowLossCo.ToString("F", format) & " " & kvp2.Value.RflowLossCo.ToString("F", format) & " " & counter
                        End If
                        generate.WriteLine(output)
                        counter = counter + 1
                    Next kvp2
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, PipeJunctions) In kvp.Value.Profile.Junctions
                        If kvp2.Value.EnterVelocityOrMassFlowRate = False Then
                            output1 = kvp2.Value.InitialLiquidVelocity.ToString("F", format) & " " & kvp2.Value.InitialVaporVelocity.ToString("F", format)
                        ElseIf kvp2.Value.EnterVelocityOrMassFlowRate = True Then
                            output1 = kvp2.Value.InitialLiquidMassFlowRate.ToString("F", format) & " " & kvp2.Value.InitialVaporMassFlowRate.ToString("F", format)
                        End If
                        If (counter < 10) Then
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.Annulus) In ChildParent.Collections.CLCS_AnnulusCollection
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, AnnulusJunctions) In kvp.Value.Profile.Junctions
                        ...
                        End If
                        If (counter < 10) Then
                            output = kvp.Value.UID & "130" & counter & " " & output1 & " " & kvp2.Value.InterphaseVelocity.ToString("F", format) & " " & counter
                        Else
                            output = kvp.Value.UID & "13" & counter & " " & output1 & " " & kvp2.Value.InterphaseVelocity.ToString("F", format) & " " & counter
                        End If
                        generate.WriteLine(output)
                        counter = counter + 1
                    Next kvp2
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.Branch) In ChildParent.Collections.CLCS_BranchCollection
                    ...
                    Else
                        ...
                        For Each kvp2 As KeyValuePair(Of Integer, BranchGeometry) In kvp.Value.BranchJunctionsGeometry.BranchGeometry
                            ...
                            generate.WriteLine(kvp.Value.UID & counter & "101 " & output9 & CInt(kvp2.Value.FromComponentVolumeNumber).ToString("D2") & "000" & kvp2.Value.FromFaceNumber & " " & output10 & CInt(kvp2.Value.ToComponentVolumeNumber).ToString("D2") & "000" & kvp2.Value.ToFaceNumber & " " & kvp2.Value.JunctionArea.ToString("F", format) & " " & kvp2.Value.FFLossCo.ToString("F", format) & " " & kvp2.Value.RFlossCo.ToString("F", format) & " " & output8 & " " & kvp2.Value.SubcooledDischargeCo.ToString("F", format) & " " & kvp2.Value.TwoPhaseDischargeCo.ToString("F", format) & " " & kvp2.Value.SuperheatedDischargeCo.ToString("F", format))
                            counter = counter + 1
                        Next kvp2
                        counter = 1
                        For Each kvp2 As KeyValuePair(Of Integer, BranchGeometry) In kvp.Value.BranchJunctionsGeometry.BranchGeometry
                            generate.WriteLine(kvp.Value.UID & counter & "201 " & kvp2.Value.LiquidMassFlow.ToString("F", format) & " " & kvp2.Value.VaporMassFlow.ToString("F", format) & " 0")
                            counter = counter + 1
                        Next kvp2
                    End If
                    univID = univID + 1
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.Turbine) In ChildParent.Collections.CLCS_TurbineCollection
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, TurbineGeometry) In kvp.Value.TurbineJunctionsGeometry.TurbineGeometry
                        ...
                        output9 = RELAP.App.GetUIDFromTag(kvp2.Value.FromComponent)
                        output10 = RELAP.App.GetUIDFromTag(kvp2.Value.ToComponent)
                        generate.WriteLine(kvp.Value.UID & counter & "101 " & output9 & CInt(kvp2.Value.FromComponentVolumeNumber).ToString("D2") & "000" & kvp2.Value.FromFaceNumber & " " & output10 & CInt(kvp2.Value.ToComponentVolumeNumber).ToString("D2") & "000" & kvp2.Value.ToFaceNumber & " " & kvp2.Value.JunctionArea.ToString("F", format) & " " & kvp2.Value.FFLossCo.ToString("F", format) & " " & kvp2.Value.RFlossCo.ToString("F", format) & " " & output8)
                        counter = counter + 1
                    Next kvp2
                    counter = 1
                    For Each kvp2 As KeyValuePair(Of Integer, TurbineGeometry) In kvp.Value.TurbineJunctionsGeometry.TurbineGeometry
                    ...
                    generate.WriteLine(kvp.Value.UID & "0300 " & kvp.Value.shaftspeed.ToString("F", format) & " " & kvp.Value.inertia.ToString("F", format) & " " & kvp.Value.shaftfriction.ToString("F", format) & " " & kvp.Value.shaftcomponentNo & " " & kvp.Value.dctripno & " 0")
                    ...
                    ElseIf kvp.Value.turbinetype.ToString = "Constant_efficiency_stage_group" Then
                        output1 = "2"
                    End If
                    generate.WriteLine(kvp.Value.UID & "0400 " & output1 & " " & kvp.Value.actualeff.ToString("F", format) & " " & kvp.Value.designfraction.ToString("F", format) & " " & kvp.Value.meanradius.ToString("F", format))
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.Separator) In ChildParent.Collections.CLCS_SeparatorCollection
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, SeparatorGeometry) In kvp.Value.SeparatorJunctionsGeometry.SeparatorGeometry
                        ...
                        If kvp2.Value.SubcooledDischargeCo = "0" Then
                            generate.WriteLine(kvp.Value.UID & counter & "101 " & output1 & CInt(kvp2.Value.FromComponentVolumeNumber).ToString("D2") & "000" & kvp2.Value.FromFaceNumber & " " & output2 & CInt(kvp2.Value.ToComponentVolumeNumber).ToString("D2") & "000" & kvp2.Value.ToFaceNumber & " " & kvp2.Value.JunctionArea.ToString("F", format) & " " & kvp2.Value.FFLossCo.ToString("F", format) & " " & kvp2.Value.RFlossCo.ToString("F", format) & " " & output8)

                        Else
                            generate.WriteLine(kvp.Value.UID & counter & "101 " & output1 & CInt(kvp2.Value.FromComponentVolumeNumber).ToString("D2") & "000" & kvp2.Value.FromFaceNumber & " " & output2 & CInt(kvp2.Value.ToComponentVolumeNumber).ToString("D2") & "000" & kvp2.Value.ToFaceNumber & " " & kvp2.Value.JunctionArea.ToString("F", format) & " " & kvp2.Value.FFLossCo.ToString("F", format) & " " & kvp2.Value.RFlossCo.ToString("F", format) & " " & output8 & " " & kvp2.Value.SubcooledDischargeCo.ToString("F", format))
                        End If
                        counter = counter + 1
                    Next kvp2
                    counter = 1
                ...
                For Each kvp As KeyValuePair(Of String, RELAP.SimulationObjects.UnitOps.HeatStructure) In ChildParent.Collections.CLCS_HeatStructureCollection
                    ...
                    If kvp.Value.HeatStructureMeshData.GapConductanceModel.ToString = True Then
                        ...
                        For Each kvp2 As KeyValuePair(Of Integer, HSGapDeformation) In kvp.Value.HeatStructureMeshData.GapDeformation
                            output = "1" & kvp.Value.UID & "00" & Counter & " " & kvp2.Value.FuelSurfaceRoughness.ToString("F7") & " " & kvp2.Value.CladdingSurfaceRoughness.ToString("F7") & " " & kvp2.Value.RadialDisplacementFission.ToString("F", format) & " " & kvp2.Value.RadialDisplacementCladding.ToString("F", format) & " " & kvp2.Value.HSnumberGapDef
                            generate.WriteLine(output)
                            Counter = Counter + 1
                        Next kvp2
                    End If
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, HSMeshDataFormat2) In kvp.Value.HeatStructureMeshData.MeshDataFormat2
                        output = "1" & kvp.Value.UID & "0" & "10" & Counter & " " & kvp2.Value.MeshInterval.ToString("F", format) & " " & kvp2.Value.IntervalNumber
                        generate.WriteLine(output)
                        Counter = Counter + 1
                    Next kvp2
                    ...
                    ElseIf kvp.Value.HeatStructureMeshData.DecayHeat = "" Then
                        Counter = 1
                        For Each kvp2 As KeyValuePair(Of Integer, HSMeshDataNoDecay) In kvp.Value.HeatStructureMeshData.MeshDataNoDecay
                            output = "1" & kvp.Value.UID & "0" & "30" & Counter & " " & kvp2.Value.SourceValue.ToString("F", format) & " " & kvp2.Value.MeshIntervalNumber
                            generate.WriteLine(output)
                            Counter = Counter + 1
                        Next kvp2
                    ...
                    Else
                        ...
                        For Each kvp2 As KeyValuePair(Of Integer, HSMeshDataWithDecay) In kvp.Value.HeatStructureMeshData.MeshDataWithDecay
                            output = "1" & kvp.Value.UID & "0" & "30" & Counter & " " & kvp2.Value.GammaAttenuationCo.ToString("F", format) & " " & kvp2.Value.MeshIntervalNumber2
                            generate.WriteLine(output)
                            Counter = Counter + 1
                        Next kvp2
                    End If
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, HSTemp1) In kvp.Value.HeatStructureMeshData.Temp1
                        output = "1" & kvp.Value.UID & "0" & "40" & Counter & " " & kvp2.Value.Temp1Temperature.ToString("F", format) & " " & kvp2.Value.Temp1MeshPointNumber
                        generate.WriteLine(output)
                        Counter = Counter + 1
                    Next kvp2
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, HSBoundaryCondTab1) In kvp.Value.HeatStructureBoundaryCond.BoundaryCondTab1
                        ...
                        Else
                            output2 = "0 0 0"
                        End If
                        output = "1" & kvp.Value.UID & "0" & "50" & Counter & " " & output2 & " " & kvp2.Value.LeftSurfaceAreaSelection & " " & kvp2.Value.LeftSurfaceArea.ToString("F", format) & " " & kvp2.Value.LeftHeatStructureNumber
                        generate.WriteLine(output)
                        Counter = Counter + 1
                    Next kvp2
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, HSBoundaryCondTab2) In kvp.Value.HeatStructureBoundaryCond.BoundaryCondTab2
                        ...
                        Else
                            output2 = "0 0"
                        End If
                        output = "1" & kvp.Value.UID & "0" & "60" & Counter & " " & output2 & " " & kvp2.Value.RightBoundaryConditionType & " " & kvp2.Value.RightSurfaceAreaSelection & " " & kvp2.Value.RightSurfaceArea.ToString("F", format) & " " & kvp2.Value.RightHeatStructureNumber
                        generate.WriteLine(output)
                        Counter = Counter + 1
                    Next kvp2
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, HSBoundaryCondTab3) In kvp.Value.HeatStructureBoundaryCond.BoundaryCondTab3
                        output = "1" & kvp.Value.UID & "0" & "70" & Counter & " " & kvp2.Value.SourceType & " " & kvp2.Value.InternalSourceMultiplier.ToString("F", format) & " " & kvp2.Value.DirectModeratorHeatingMultiplierLeft.ToString("F", format) & " " & kvp2.Value.DirectModeratorHeatingMultiplierRight.ToString("F", format) & " " & kvp2.Value.SourceHeatStructureNumber
                        generate.WriteLine(output)
                        Counter = Counter + 1
                    Next kvp2
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, HSBoundaryCondTab4) In kvp.Value.HeatStructureBoundaryCond.BoundaryCondTab4
                        output = "1" & kvp.Value.UID & "0" & "80" & Counter & " " & kvp2.Value.leftHeatedEquivalentDiameter.ToString("F4") & " " & kvp2.Value.LeftHeatedLengthForward.ToString("F", format) & " " & kvp2.Value.LeftHeatedLengthReverse.ToString("F", format) & " " & kvp2.Value.leftGridSpacerLengthForward.ToString("F", format) & " " & kvp2.Value.leftGridSpacerLengthReverse.ToString("F", format) & " " & kvp2.Value.leftGridLossCoefficientForward.ToString("F", format) & " " & kvp2.Value.leftGridLossCoefficientReverse.ToString("F", format) & " " & kvp2.Value.leftLocalBoilingFactor.ToString("F", format) & " " & kvp2.Value.leftNaturalCirculationLength.ToString("F4") & " " & kvp2.Value.leftPitchtoDiameterRatio.ToString("F", format) & " " & kvp2.Value.leftFoulingFactor.ToString("F", format) & " " & kvp2.Value.leftAddHeatStructureNumber
                        generate.WriteLine(output)
                        Counter = Counter + 1
                    Next kvp2
                    ...
                    For Each kvp2 As KeyValuePair(Of Integer, HSBoundaryCondTab5) In kvp.Value.HeatStructureBoundaryCond.BoundaryCondTab5
                        output = "1" & kvp.Value.UID & "0" & "90" & Counter & " " & kvp2.Value.rightHeatedEquivalentDiameter.ToString("F4") & " " & kvp2.Value.rightHeatedLengthForward.ToString("F", format) & " " & kvp2.Value.rightHeatedLengthReverse.ToString("F", format) & " " & kvp2.Value.rightGridSpacerLengthForward.ToString("F", format) & " " & kvp2.Value.rightGridSpacerLengthReverse.ToString("F", format) & " " & kvp2.Value.rightGridLossCoefficientForward.ToString("F", format) & " " & kvp2.Value.rightGridLossCoefficientReverse.ToString("F", format) & " " & kvp2.Value.rightLocalBoilingFactor.ToString("F", format) & " " & kvp2.Value.rightNaturalCirculationLength.ToString("F4") & " " & kvp2.Value.rightPitchtoDiameterRatio.ToString("F", format) & " " & kvp2.Value.rightFoulingFactor.ToString("F", format) & " " & kvp2.Value.rightAddHeatStructureNumber
                        generate.WriteLine(output)
                        Counter = Counter + 1
                    Next kvp2
                Next kvp
                ...
                Try
                    ...
                    For j = 0 To frm.dgv1.Rows.Count - 2
                        ...
                        If dgvrow.Cells(5).Value = "Both" Then
                            ...
                            output = "205" & cccc.ToString("D4") & "0" & " " & dgvrow.Cells(0).Value & " " & dgvrow.Cells(1).Value.ToLower() & " " & CDbl(dgvrow.Cells(2).Value).ToString("F", format) & " " & CDbl(dgvrow.Cells(3).Value).ToString("F", format) & " " & dgvrow.Cells(4).Value & " " & "2" & " " & dgvrow.Cells(6).Value & " " & dgvrow.Cells(7).Value
                            '                    Else
